    ...
    local function t_ExecuteFnWithArgsForPeers(funcname,waittime,returnafterfinish,ForPeers,...)
        ...
        if not ts.GameSetup.GetIsMultiPlayerGame() or (g_CoopCountResSerp~=nil and g_CoopCountResSerp.Finished==true and g_CoopCountResSerp.TotalCount==1) then
          if type(ForPeers)~="table" or g_LTL_Serp.table_contains_value(ForPeers,g_PeersInfo_Serp.PeerInt) then
            g_LTL_Serp.modlog("t_ExecuteFnWithArgsForPeers executing code for the only active human (change code if this is wrong ForPeers is not properly checked in this case!). ForPeers : "..tostring(ForPeers),ModID)
            local function DoSingleExecution(funcname,args)
              local func = g_LTL_Serp.myeval(funcname)
              local success, ret = xpcall(func,g_LTL_Serp.log_error,table.unpack(args))
              if success==false then
                g_LTL_Serp.modlog("ERROR _DoTheExecutionFor while trying to call funcname "..tostring(funcname).." error: "..tostring(ret),ModID)
              end
              return ret
            end
            if returnafterfinish then -- execute in current thread and block it (return after finished)
              return DoSingleExecution(funcname,args)
            else -- execute in new thread (and return before it is finished)
              g_LTL_Serp.start_thread("DoSingleExecution_random_",ModID,DoSingleExecution,funcname,args)
            end
            return
          end
        end
        ...
        local function DoExecuteFnWithArgsForPeers(inhex,waittime)
            ...
            if status==false then
              g_LTL_Serp.modlog("t_ExecuteFnWithArgsForPeers ERROR : "..tostring(err),ModID)
              error(err) 
            end
        ...
        if returnafterfinish then -- execute in current thread and block it (return after finished)
          ...
          g_LTL_Serp.start_thread("DoExecuteFnWithArgsForPeers_random_",ModID,DoExecuteFnWithArgsForPeers,inhex,waittime)
    ...
    local function _DoTheExecutionFor(FromPeerInt)
      ...
      if type(intable) =="table" then
        ...
        if ShouldIExecute then
          ...
          if success==false then
            g_LTL_Serp.modlog("ERROR _DoTheExecutionFor while trying to call funcname "..tostring(intable.funcname).." error: "..tostring(err),ModID)
          end
            -- g_LTL_Serp.modlog("DoTheExecutionFor after call",ModID)
        end
      else -- sollte jeder auslesen können
        g_LTL_Serp.modlog("ERROR _DoTheExecutionFor intable ist keine table (offenbar Nameable nicht vorhanden, oder invalide Infos versucht reinzuschreiben): "..tostring(intable),ModID)
      end
    end
