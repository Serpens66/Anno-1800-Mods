...
-- TODO:
 -- man sieht die ganzen Helper Nameables im Statisk-schiffsmenü?!
-- mist.. weder im asset noch im <Name>StatisticsMenu</Name> sehe ich was wie man diese
 -- in der shiplist hiden könnte...
-- evlt könnte man auf owner verzichten? dann würde es bestimmt nicht angezeigt
-- mal testen ohne Owner, aber dafür 4 Objekte auch für text-embed spawnen,
 -- damit GetCoopPeersAtMarker zuverlässig funzen kann, auch wenn alle Humans es zeitgleich machen
 -- wobei es durch die fehlende wartezeit lokal alles richtig sein sollte und nicht durch anderen spieler überschrieben wird
 -- evlt testen
...
local function FindHelperOIDs(onlysavehelper)
  ...
  if next(ObjectFinderSerp.ObjectFinderCacheSerp["Nameable_Helper_OIDs"])==nil or (SaveLuaStuff_Serp.Helper_OID==nil and onlysavehelper) then
    ...
    if ts.Participants.GetParticipant(HelperOwner).ProfileCounter.Stats.GetCounter(0,0,next(HelperGUIDs),3)>0 then -- if 1500004625 exists, all others must exist too, because they are spawned in the very same xml code
      ...
      print("spawn von helper objects hat nicht geklappt?!")
    end
  else
 ...
 local function DoForSessionGameObject(ts_embed_string,doreturnstring)
  if doreturnstring then -- we want to get what the textembed returns, but game.TextSourceManager.setDebugTextSource does not return anything. I only know a workarkund to get it, by setting and reading out the name of a namable helper object
    ...
    if helper_OID~=nil then
      ...
      print("DoForSessionGameObject: Nameable_Helper_OIDs EmbedHelper for helper object GUID 1500004620 is empty for player "..tostring(CurrentParticipantID)..". Call FindHelperOIDs first and check the prints if it correctly spawned")
    end
...
local function GetFertilities_CurrentSession(AreaID)
  local num_fertilities = ObjectFinderSerp.DoForSessionGameObject("[Area AreaFromID("..tostring(AreaID)..") Fertilities Count]",true)
  local Fertilities = {}
  if num_fertilities~=nil and num_fertilities~="nil" then
    num_fertilities = tonumber(num_fertilities)
    if num_fertilities~=nil and num_fertilities>0 then
      local Fertility_GUID
      for i=0,num_fertilities-1 do
        Fertility_GUID = ObjectFinderSerp.DoForSessionGameObject("[Area Current Fertilities At("..tostring(i)..") Guid]",true)
        if Fertility_GUID~=nil and Fertility_GUID~="nil" then
          Fertilities[Fertility_GUID] = true -- use it as key, so you can easily check for a specific fertility without looping over the list
        end
      end
    end
  end
...
local function GetCoopPeersAtMarker_StatisticsMenu()
  local RefGuid = 176 -- for ships it is 119 (I assume it is UIState?)
  local RefOid = ObjectFinderSerp.IDConverter.get_OID(session.getSelectedFactory())
  local count = ObjectFinderSerp.DoForSessionGameObject("[Online GetCoopPeersAtMarker("..tostring(RefGuid)..","..tostring(OID)..") Count]",true)
  local peerints = {}
  if count~=nil and count~="nil" then
    ...
    if count~=nil and count>0 then
      local peerint
      for i=0,count-1 do
        peerint = ObjectFinderSerp.DoForSessionGameObject("[Online GetCoopPeersAtMarker("..tostring(RefGuid)..","..tostring(OID)..") At("..tostring(i)..")]",true)
        if peerint~=nil and peerint~="nil" then
          peerints[peerint] = true -- use it as key, so you can easily check for a specific without looping over the list
        end
      end
    end
  end
...
-- TODO:
-- "Lodes" sind glaube ich Erze der Insel
 -- sowas auch für GetCargo/Sockets für Schiffe machen. Solte dank SessionGameObject für Schiffe in egal welcher Session funktionieren
 -- und für Factories und Monument können wir die Buffs auslesen die auf sie wirken mit:
  -- [Selection Object Factory ProductivityUpgradeList At(0) Guid] (bzw Monument) , man bekommt ein "CItemAsset"
 -- leider klappt das nicht für schiffe oderso... (und testen obs nur klappt für Buffs die ProductivityUpgrade s geben)
  -- Toolone ItemBuffs und ItemBuffsSpecial kann man glaube ich nur eine Buff/Item GUID übergeben und bekommt dann 
   -- die einzelnen effekte des buffs?
...
-- PROBLEM TODO:
-- ts.GetGameObject(HelperOID).Nameable.SetName(infostring) für fremde sessions funktioniert im Singleplayer
 -- unabhängig von der Session.
-- Aber wenn der ausführende Spieler in einer anderen Session als das Objekt ist, dann klappt es nur für ihn selbst.
 -- für andere Spieler im MP bleibt der alte Namen bestehen.
 -- nur wenn Spieler und Objekt in selber Session sind, wird Namenswechsel "synchronsiert" zwischenden spielern...
-- trifft das problem auch auf GetName zu? Nein, GetName klappt von jeder Session aus, auch wenn das schiff von anderem spieler in anderer session umbenannt wurde
  -- (sofern der umbenenner halt in derselben session ist)
  
  -- DoForSessionGameObject löst das eine Problem zwar,
   -- aber ein weiteres Problem ist:
   -- Nachdem man SetName macht, wird das lokal für den Spieler sofort instant hinterlegt und ein GetName imselben Tick returned das korrekte.
  -- Doch danach scheint noch irgendwas stattzufinden, selbst im Singleplayer, was den Namen synchronisiert oderso.
  -- Das heißt selbst im Singleplayer kommt sowas bei rum:
    -- ts.GetGameObject(OID).Nameable.SetName("234")
    -- coroutine.yield()
    -- ts.GetGameObject(OID).Nameable.SetName("2345")
    -- print(ts.GetGameObject(OID).Nameable.GetName()) -- prints 2345
    -- coroutine.yield()
    -- print(ts.GetGameObject(OID).Nameable.GetName()) -- prints 234
    -- coroutine.yield()
    -- print(ts.GetGameObject(OID).Nameable.GetName()) -- prints 234
    -- coroutine.yield()
    -- coroutine.yield()
    -- print(ts.GetGameObject(OID).Nameable.GetName()) -- prints 2345
  -- man muss sich also wieder mit yield rumschlagen und erst nach x yields ists dann final eingetragen -.-
  -- Und im Multiplayer können andere Spieler es halt überhaupt erst nach diesen x yields sehen, dass was geändert wurde...
  -- Dadurch dass es lokal sofort verfügbar ist, kann man auch kein while true machen der wartet bis name nicht mehr "nil" ist oderso,
   -- weil damit alles funzt ja alle spieler gleich viel yield machen müssen..
   -- und einfach dann 3 yields hinklatschen ist auch nicht sicher, denn die PCs der Spieler können untersch. schnell sein im übertragen der namen, 
    -- der eine hat es "schon" nach 3 yields, der andere braucht 4 oder mehr -.-
    -- Dh. sicher wäre es so ab 20 yields, was 2 ingame sekunden sind (bei langsamem Spielfluss also länger in echtzeit), was doch recht lang ist..

-- für ShareLuaInfo ist das ziemlich doof,weil das eig. schnell gehen soll..
 -- für SaveStuff ist es nicht soo schlimm, weil man da in der regel keinen zeitdruck hat,
  -- ABER wenn zufällig dann während der ~2 sekunden die das noch am Namen syncen ist das Spiel beendet wird,
   -- und die Namen dann für die Spieler untersch. sind, wird es natürlich zwangsweise oft zu desync kommen...
   -- Dazu evlt ein ShareLuaInfo mit 30 yields oderso nutzen um ab und zu die saveTable abzugeleichen?
  -- Wobei ein ShareLuaInfo was 2-3 sekunden dauert auch nicht praktisch in der Verwendung ist, weil ja mehrere skripte es evlt. nutzen wollen..

local function ShareLuaInfo(infostring)
  ...
  if HelperOID~=nil then
    ...
    -- ts.GetGameObject(HelperOID).Nameable.SetName(infostring) -- in multiplayer this only works for other players too if we are in the same session like the object... (in SP it also works in differnt session)
...
local function GetSharedLuaInfo(HumanID)
  local HelperOID
  if HumanID==0 then
    HelperOID = ObjectFinderSerp.ObjectFinderCacheSerp["Nameable_Helper_OIDs"]["Human0Share"]
  elseif HumanID==1 then
    HelperOID = ObjectFinderSerp.ObjectFinderCacheSerp["Nameable_Helper_OIDs"]["Human1Share"]
  elseif HumanID==2 then
    HelperOID = ObjectFinderSerp.ObjectFinderCacheSerp["Nameable_Helper_OIDs"]["Human2Share"]
  elseif HumanID==3 then
    HelperOID = ObjectFinderSerp.ObjectFinderCacheSerp["Nameable_Helper_OIDs"]["Human3Share"]
  end
  if HelperOID~=nil then
...
-- TODO:
-- Syntax error: "data/scripts_serp/objectfinder/objectfinder.lua":507: too many C levels (limit is 200) in function at line 418 near '"done"'
-- keine ahnung was das auslöst, kommt jetzt auch in singleplayer..
      ...
      for IslandID=FromIslandID,ToIslandID do
        for AreaIndex=FromAreaIndex,ToAreaIndex do
          if not ((IslandID==0 and AreaIndex~=0) or (IslandID~=0 and AreaIndex==0)) then -- on water both are 0. on islands none of them is 0, so do not allow only one of them being 0.
            ...
            if ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]~=false then
              if withyield then
                coroutine.yield()
              end
              ...
              AreaOwnerName = ts.Area.GetAreaFromID(AreaID).OwnerName
              if IslandID~=0 and (AreaOwnerName=="" and ts.Area.GetAreaFromID(AreaID).Owner==0) then -- invalid area. while for unsettled area owner will be -1
                ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID] = false -- remember this Area as invalid, so we don't have to check it ever again.
              end
              Kontor_OIDtable = ts.Area.GetAreaFromID(AreaID).KontorID
              Kontor_OID = OIDtableToOID(Kontor_OIDtable)
              if IslandID==0 or AreaOwnerName~="" then -- if not Water (IslandID==0), make sure Area is owned by someone, which also skips invalid Areas, important for performance. (OwnerName ist better than Owner here, because invalid areas are nullpointer and would return 0 for Owner, which also might be Human0 on valid areas)
                if ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]== nil then
                  ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID] = {LeereObIDs={},LowObID=nil,HighObID=nil}
                end
                if IslandID~=0 then
                  ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"] = Kontor_OIDtable["ObjectID"] -- Kontor should be the first building on every owned island, so it is enough to start there with counting
                end
                if FromObjectID==nil and ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"]~=nil then
                  FromObjectID_ = ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"]
                end
                -- if ToObjectID==nil and ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["HighObID"]~=nil then
                  -- ToObjectID_ = ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["HighObID"] + 10000 -- wenn gesetzt, MUSS HighObID natürlich regelmäßig abgefragt werden, müssten wir also zb. selbst alle 5 min oderso hier anstoßen... 
                -- end
                objectcount = 0
                ...
                for ObjectID=FromObjectID_,ToObjectID_ do
                  if not ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LeereObIDs"][ObjectID] then
                    filterresult = nil
                    OID = OIDtableToOID({ObjectID=ObjectID,AreaID=AreaID})
                    if withyield then
                    ...
                    -- else -- just empty meanwhile. wont be used by the game anymore, so save this in our cache...  too inefficient to save every single on. need to find a better way...
                      -- ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LeereObIDs"][ObjectID]=true
                    end
                  end
                end
                if LowestObjectID~=nil and (ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"]==nil or LowestObjectID > ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"]) then  
                  ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]["LowObID"] = LowestObjectID
                end
      ...
      if session_ok then -- not really needed if we find objects and reach the ObjectFilter call (then use next_SessionID), but eg. if we want to only check Areas then we wont find Neutral object and reach ObjectFilter quite late. So use SessionFilter to sort out unwanted session early to save performance
        if SessionIsCurrent then
          ...
          -- TODO: testen ob das so problemlos funzt
           -- wenn man in der session ein "done" returned, kommt das hier in dieser FKT nicht an...
        else
          for IslandID=FromIslandID,ToIslandID do
            for AreaIndex=FromAreaIndex,ToAreaIndex do
              if not ((IslandID==0 and AreaIndex~=0) or (IslandID~=0 and AreaIndex==0)) then -- on water both are 0. on islands none of them is 0, so do not allow only one of them being 0.
                AreaID = AreatableToAreaID({SessionID=SessionID,IslandID=IslandID,AreaIndex=AreaIndex})
                if ObjectFinderSerp.ObjectFinderCacheSerp["ObIDs"][SessionID][AreaID]~=false then -- dont check Areas we already marked as invalid
                  if withyield then
                    coroutine.yield()
                  end
                  FromObjectID_ = FromObjectID or 1
...
ObjectFinderSerp = ObjectFinderSerp or {
  ...
  GetFertilities_CurrentSession = GetFertilities_CurrentSession,
  GetCoopPeersAtMarker_StatisticsMenu = GetCoopPeersAtMarker_StatisticsMenu,
  -- object loopers
