-- Big thanks also to Taube and nyk for helping and finding out important information about how ObjectIDs and so on work.
  ...
  -- Uses CompanyName from Scenario3_Challenger3 for DoForSessionGameObject (they exist in every vanilla game in every session)
   -- Uses CompanyName from Scenario3_Challenger4 for FnViaTextEmbed functionality in LuaTools shared mod
   ...
   -- CompanyName Scenario3_Challenger1 and Scenario3_Challenger2 used in shared_PopUp mod.
    -- Nameable from Scenario3_Challenger1+Scenario3_Challenger2 used here in addition to Nameable from the following:
...
-- Anstatt system.start() nutze lieber meine Funktion g_AnnoTools.start_thread(threadname,ModID,fn,...). Denn diese hat 
...
if g_LuaScriptBlockers[ModID]==nil then
    ...
    g_AnnoTools.modlog("objectfinder.lua called",ModID)
    ...
    local function t_SaveCache()
      ...
      g_AnnoTools.modlog("SaveCache called ",ModID)
      if g_ObjectFinderSerp.ObjectFinderCacheSerp.Changed then
        g_AnnoTools.start_thread("call t_SyncCacheToEveryone",ModID,g_ObjectFinderSerp.t_SyncCacheToEveryone)
      end
      g_AnnoTools.waitForTimeDelta(6000) -- problem is depending on usage t_ExecuteFnWithArgsForPeers can take between 4 and endless seconds... but we assume its max 6 seconds. and if it takes longer, it will be saved next time
    ...
    local function StartShareThread()
      g_AnnoTools.start_thread("StartShareThread",ModID,function()
        -- g_AnnoTools.modlog("StartShareThread "..tostring(ts.GameClock.CorporationTime),ModID)
        ...
        while task~=nil do
          if type(task)=="table" and task.infostring~=nil then
            ...
            -- g_AnnoTools.modlog("StartShareThread before DoneVariableString "..tostring(ts.GameClock.CorporationTime),ModID)
            local DoneVariable,last_key = g_LuaTools.myeval(task.DoneVariableString,true)

            -- g_AnnoTools.modlog("StartShareThread after DoneVariableString "..tostring(ts.GameClock.CorporationTime),ModID)
            local PID_OID = nil
            ...
            -- g_AnnoTools.modlog("StartShareThread after GetAllLoadedSessionsParticipants "..tostring(ts.GameClock.CorporationTime),ModID)
            for SessionID,session_pids in pairs(sessionparticipants) do
              PID_OID = session_pids[SharePID].OID
              local status,err = pcall(g_AnnoTools.DoForSessionGameObject,"[MetaObjects SessionGameObject("..tostring(PID_OID)..") Nameable Name("..tostring(infostring)..")]")
              if status==false then
                g_AnnoTools.modlog("ERROR : "..tostring(err),ModID)
                error(err) 
              end
            end
            -- g_AnnoTools.modlog("StartShareThread after DoForSessionGameObject "..tostring(ts.GameClock.CorporationTime),ModID)
            g_AnnoTools.waitForTimeDelta(waittime) -- wait before we return, so after we return the caller can directly continue to GetSharedLuaInfo
            if DoneVariable[last_key]~=nil then
              DoneVariable[last_key] = infostring -- signal that we are finished setting up the Nameable
            end
            g_AnnoTools.waitForTimeDelta(1000) -- wait another second before reusing the Nameable, so clients can do GetSharedLuaInfo
            DoneVariable[last_key] = false
            -- g_AnnoTools.modlog("StartShareThread task done "..tostring(ts.GameClock.CorporationTime),ModID)
          end
          task = table.remove(g_ObjectFinderSerp.ShareQueue,1)
        end
        -- g_AnnoTools.modlog("StartShareThread Stop "..tostring(ts.GameClock.CorporationTime),ModID)
    ...
    local function t_ShareLuaInfo(infostring,waittime,DoneVariableString)
        ...
        -- g_AnnoTools.modlog("t_ShareLuaInfo before yield "..tostring(ts.GameClock.CorporationTime),ModID)
        ...
        -- g_AnnoTools.modlog("t_ShareLuaInfo after yield "..tostring(ts.GameClock.CorporationTime),ModID)
        ...
        -- g_AnnoTools.modlog("t_ShareLuaInfo table.insert "..tostring(ts.GameClock.CorporationTime),ModID)
        if not g_ObjectFinderSerp.ShareLoopIsRunning then
          g_ObjectFinderSerp.ShareLoopIsRunning = true
          -- g_AnnoTools.modlog("t_ShareLuaInfo ShareLoopIsRunning false, now true "..tostring(ts.GameClock.CorporationTime),ModID)
          g_ObjectFinderSerp.StartShareThread()
        end
        -- g_AnnoTools.modlog("t_ShareLuaInfo nach thread starten "..tostring(ts.GameClock.CorporationTime),ModID)
        return true
    end
    ...
    local function t_ExecuteFnWithArgsForPeers(funcname,waittime,returnafterfinish,ForPeers,...)
        g_AnnoTools.modlog("t_ExecuteFnWithArgsForPeers start "..tostring(funcname).." "..tostring(ts.GameClock.CorporationTime),ModID)
        ...
        local function DoExecuteFnWithArgsForPeers(inhex,waittime)
            -- g_AnnoTools.modlog("DoExecuteFnWithArgsForPeers before share "..tostring(ts.GameClock.CorporationTime),ModID)
            local status,err = pcall(g_ObjectFinderSerp.t_ShareLuaInfo,inhex,waittime,"g_ObjectFinderSerp.ExecuteDone")
            if status==false then
              g_AnnoTools.modlog("ERROR : "..tostring(err),ModID)
              error(err) 
            end
            -- g_AnnoTools.modlog("DoExecuteFnWithArgsForPeers shared "..tostring(ts.GameClock.CorporationTime),ModID)
            while g_ObjectFinderSerp.ExecuteDone~=inhex do -- wait for it to finish (after that we have 1 sec to fetch the info)
              coroutine.yield()
            end
            -- g_AnnoTools.modlog("DoExecuteFnWithArgsForPeers ExecuteDone "..tostring(ts.GameClock.CorporationTime),ModID)
            ...
            -- g_AnnoTools.modlog("DoExecuteFnWithArgsForPeers unlocked "..tostring(ts.GameClock.CorporationTime),ModID)
            g_AnnoTools.waitForTimeDelta(500) -- wait shortly to make sure all coop players executed this
        end
    ...
    local function DoTheExecutionFor(FromPeerInt)
      g_AnnoTools.modlog("DoTheExecutionFor FromPeerInt "..tostring(FromPeerInt).." "..tostring(ts.GameClock.CorporationTime),ModID)
      ...
      if type(intable) =="table" then
        -- g_AnnoTools.modlog(intable.funcname,ModID)
        ...
        elseif type(ForPeers)=="string" then
          ...
          elseif ForPeers=="Human3_FirstPeer" then
            ...
            g_AnnoTools.modlog("ERROR DoTheExecutionFor (t_ExecuteFnWithArgsForPeers): invalid -ForPeers- was provided: "..tostring(ForPeers).." . Will execute for everyone now",ModID)
            ShouldIExecute = true
          end
        ...
        -- g_AnnoTools.modlog("DoTheExecutionFor before ShouldIExecute",ModID)
        if ShouldIExecute then
          -- g_AnnoTools.modlog("DoTheExecutionFor yes ShouldIExecute",ModID)
          func = g_LuaTools.myeval(intable.funcname)
          -- g_AnnoTools.modlog("func call "..tostring(ts.GameClock.CorporationTime),ModID)
          local success, err = pcall(func,table.unpack(intable.args))
          if success==false then
            g_AnnoTools.modlog("ERROR while trying to call funcname "..tostring(intable.funcname).." error: "..tostring(err),ModID)
          end
            -- g_AnnoTools.modlog("DoTheExecutionFor after call",ModID)
        end
      end
    end
    ...
    local function GetHighestObIDsLocalPlayerCurrentSessionByProperty(Property)
      ...
      for OID,objinfo in pairs(ret.Objects) do
        local OIDtable = g_AnnoTools.OIDToOIDtable(OID)
        local ObjectID = OIDtable.ObjectID
        local AreaID = g_AnnoTools.AreatableToAreaID(OIDtable.AreaID)
      ...
      g_AnnoTools.start_thread("reset HighObID",ModID,function()
    ...
    local function GetCurrentSessionObjectsFromLocaleByProperty(Property)
      ...
      elseif type(Property)=="string" then
        PropertyID = g_AnnoTools.PropertiesStringToID[Property]
      end
      local GUID,OID
      local Objects = {}
        ...
        if type(userdatas)=="table" and #userdatas>0 then
          for k,userdata in ipairs(userdatas) do
            if userdata~=nil then -- is never nil, but nullpointer if not assigned. but here it should always be assigned
              OID = g_AnnoTools.get_OID(userdata)
    ...
    local function IsLoadedSessionByID(SessionID,dontusecache)
      ...
      local SessionGuid
      local AreaID = g_AnnoTools.AreatableToAreaID({SessionID=SessionID,IslandID=IslandID,AreaIndex=AreaIndex})
      for ObjectID=1,10 do -- to find  Neutral SessionParticipant checking the first 10 ObjectIDs should be enough
        OID = g_AnnoTools.OIDtableToOID({ObjectID=ObjectID,AreaID=AreaID,EditorFlag=0,EditorChunkID=0})
        -- g_AnnoTools.modlog("IsLoadedSessionByID before GetGameObject ObjectID "..ObjectID,ModID)
        local GUID = ts.GetGameObject(OID).GUID
        -- g_AnnoTools.modlog("IsLoadedSessionByID after GetGameObject ObjectID "..ObjectID,ModID)
    ...
    local function GetAllLoadedSessionsParticipants(PIDs,sSessionGuid,sSessionID)
      -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants called",ModID) -- testing if it loads in savegame correctly
      local PIDs_GUIDs = {}
      ...
      -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants befire IsLoadedSessionByID",ModID)
      for SessionID=FromSessionID,ToSessionID do
        local SessionGuid = g_ObjectFinderSerp.IsLoadedSessionByID(SessionID)
        if SessionGuid~=nil and (sSessionGuid==nil or sSessionGuid=="First" or sSessionGuid==SessionGuid) then
          sessionparticipants[SessionID] = {}
          
          -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants before dothesearch?",ModID) -- testing if it loads in savegame correctly
          ...
          -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants after dothesearch?",ModID) -- testing if it loads in savegame correctly
          if dothesearch then
            -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants NOT used Cache :(",ModID) -- testing if it loads in savegame correctly
            update_done = true
            local IslandID,AreaIndex = 0,0
            local AreaID = g_AnnoTools.AreatableToAreaID({SessionID=SessionID,IslandID=IslandID,AreaIndex=AreaIndex})
            for ObjectID=1,100 do
            ...
            -- g_AnnoTools.modlog("GetAllLoadedSessionsParticipants used Cache :)",ModID) -- testing if it loads in savegame correctly
                    ...
                    -- if IslandID==0 or AreaOwnerName~="" then -- if not Water (IslandID==0), make sure Area is owned by someone
                      ...
                      -- if IslandID~=0 and Kontor_OIDtable["ObjectID"]~=nil and Kontor_OIDtable["ObjectID"]~=0 then -- assuming the KontorObjectID is always the lowest on an island, it makes sense to set LowObID to this value. If we assume that there may be exceptions to this, then we can not use it at all...
                        -- if CacheObIDsSessionID[AreaID].LowObID~=nil and Kontor_OIDtable["ObjectID"]~=CacheObIDsSessionID[AreaID].LowObID then
                          -- g_AnnoTools.modlog("GetCurrentSessionObjectsFromAnyone KontorObjectID "..tostring(Kontor_OIDtable["ObjectID"]).." is unequal LowObID "..tostring(CacheObIDsSessionID[AreaID].LowObID).." ?! (may only happen on island takeover?)",ModID)
                        -- end
    ...
    local function DoesMaxObjIdHelperExists(AreaID,AreaOwner,MaxObjIdWalkableHelperGuid,MaxObjIdAreaHelperGuid)
      ...
      local Areatable = g_AnnoTools.AreaIDToAreatable(AreaID)
    ...
    local function GetAnyObjectsFromAnyone(myargs)
      g_AnnoTools.modlog("Start GetAnyObjectsFromAnyone",ModID)
      ...
      for SessionID=FromSessionID,ToSessionID do
        -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone vor IsLoadedSessionByID SessionID "..SessionID,ModID)
        local CheckingSessionGuid = g_ObjectFinderSerp.IsLoadedSessionByID(SessionID)
        if CheckingSessionGuid~=nil then
          -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone nach IsLoadedSessionByID CheckingSessionGuid "..CheckingSessionGuid,ModID)
          ...
          -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone nach SessionFilter SessionID "..SessionID.." session_ok: "..tostring(session_ok),ModID)
          if session_ok then
              ...
              for AreaIndex=FromAreaIndex,ToAreaIndex do
                if not ((IslandID==0 and AreaIndex~=0) or (IslandID~=0 and AreaIndex==0)) then -- on water both are 0. on islands none of them is 0, so do not allow only one of them being 0.
                  ...
                  if not CacheObIDsSessionID[AreaID]["invalid"] then
                    ...
                    -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone nach AreaOwnerName SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                    if not CacheObIDsSessionID[AreaID]["invalid"] then
                      if withyield then
                        coroutine.yield()
                      end
                      if not IsIsland or AreaOwnerName~="" then -- Water or Island with Owner?
                        -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone valid Area, vor DoesMaxObjIdHelperExists SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                        local MaxObjIdHelperExists = g_ObjectFinderSerp.DoesMaxObjIdHelperExists(AreaID,AreaOwner)
                        -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone valid Area, nach DoesMaxObjIdHelperExists SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                        ...
                        if SessionIsCurrent then
                          if IsIsland then
                            ...
                            Kontor_OIDtable = ts.Area.GetAreaFromID(AreaID).KontorID
                            Kontor_OID = g_AnnoTools.OIDtableToOID(Kontor_OIDtable)
                            -- filling it like this makes sure that outdated information is automatically overwritten
                            g_ObjectFinderSerp.ObjectFinderCacheSerp.Kontor_OIDs[SessionID][AreaID] = {Kontor_OID=Kontor_OID,Kontor_OIDtable=Kontor_OIDtable,AreaOwner=AreaOwner,AreaOwnerName=AreaOwnerName}
                            ...
                            if t_userdata~=nil then
                              local u_OID = g_AnnoTools.get_OID(t_userdata,"string")
                              if tostring(u_OID) ~= tostring(Kontor_OID) then
                                g_AnnoTools.modlog("ERROR WARNING GetAnyObjectsFromAnyone Kontor_OID "..tostring(Kontor_OID).." is unequal u_OID "..tostring(u_OID).." ?! Guess ID Converter is wrong?",ModID)
                              end
                            end
                            ...
                            if Kontor_OIDtable["ObjectID"]~=nil and Kontor_OIDtable["ObjectID"]~=0 then -- assuming the KontorObjectID is always the lowest on an island, it makes sense to set LowObID to this value. If we assume that there may be exceptions to this, then we can not use it at all...
                              if CacheObIDsSessionID[AreaID].LowObID~=nil and Kontor_OIDtable["ObjectID"]~=CacheObIDsSessionID[AreaID].LowObID then
                                g_AnnoTools.modlog("WARNING GetAnyObjectsFromAnyone KontorObjectID "..tostring(Kontor_OIDtable["ObjectID"]).." is unequal LowObID "..tostring(CacheObIDsSessionID[AreaID].LowObID).." ?! (may only happen on island takeover?)",ModID)
                              end
                              CacheObIDsSessionID[AreaID].LowObID = Kontor_OIDtable["ObjectID"] -- Kontor should be the first building on every owned island, so it is enough to start there with counting
                            end
                          end
                        ...
                        -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone vor Objects Loop SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                        local objectcount = 0
                        local LowestObjectID = nil
                        for ObjectID=FromObjectID_,ToObjectID_ do
                          filterresult = nil
                          local OID = g_AnnoTools.OIDtableToOID({ObjectID=ObjectID,AreaID=AreaID,EditorFlag=0,EditorChunkID=0})
                          if withyield then
                            objectcount = objectcount + 1
                            if objectcount % 10000 == 0 then
                              coroutine.yield()
                          ...
                          local GUID = g_AnnoTools.GetGameObjectPath(OID,"GUID")
                          ...
                          -- else -- string
                            -- GUID = g_AnnoTools.DoForSessionGameObject("[MetaObjects SessionGameObject("..tostring(OID)..") GUID]",true)
                          -- end
                          if GUID~=0 and GUID~=nil then -- GUID==0 means nullpointer, does not have an object
                            local SessionGuid = CheckingSessionGuid
                            -- local ParticipantID = type(OID)=="number" and ts.GetGameObject(OID).Owner or g_AnnoTools.DoForSessionGameObject("[MetaObjects SessionGameObject("..tostring(OID)..") Owner]",true)
                            local ParticipantID = g_AnnoTools.GetGameObjectPath(OID,"Owner")
                          ...
                          elseif Kontor_OIDtable~=nil and Kontor_OIDtable["ObjectID"]==ObjectID then -- if it claims to have a kontor, but its GUID is 0, then just skip this area
                            -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone Skipping Area, its preplaced Kontor (ThridParty) SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                            CacheObIDsSessionID[AreaID]["invalid"] = "EditorObject" -- remember this Area as invalid, so we don't have to check it ever again.
                            break -- this happens for Editor Objects, eg. Kontor from third parties. They have in fact OID 9223413826886612345 and EditorFlag=1 which makes the number much higher. But lua can not handle this high number (bint?) anyways, trying to get GUID from 9223413826886612345 just results in an error.  -- so no suport for Editor Objects.
                          end
                        end
                        -- g_AnnoTools.modlog("GetAnyObjectsFromAnyone nach Objects Loop SessionID "..SessionID.." AreaID: "..tostring(AreaID).." AreaOwnerName: "..tostring(AreaOwnerName),ModID)
                        if LowestObjectID~=nil and (CacheObIDsSessionID[AreaID].LowObID==nil or LowestObjectID > CacheObIDsSessionID[AreaID].LowObID) then -- checking > here is correct, because the obj only can get bigger, never lower, eg. when taking over an island
                          CacheObIDsSessionID[AreaID].LowObID = LowestObjectID
                        end
                        if filterresult~=nil and type(filterresult)=="table" and (filterresult["next_SessionID"] or filterresult["done"]) then
                      ...
                      elseif IsIsland and AreaOwner==-1 then -- valid but not owned by someone (anymore?)
                        g_ObjectFinderSerp.ObjectFinderCacheSerp.Kontor_OIDs[SessionID][AreaID] = nil -- set it to nil to potentially update it
                      end
                    end
                  end
                end
      ...
      g_AnnoTools.modlog("Ende GetAnyObjectsFromAnyone",ModID)
      return Objects
    end
    ...
    local function myObjectFilter(OID,GUID,userdata,SessionGuid,ParticipantID,AreaID,SessionID,IslandID,AreaIndex,ObjectID,AreaOwner,Kontor_OID)
      local AreaOwner = AreaOwner or g_AnnoTools.DoForSessionGameObject("[MetaObjects SessionGameObject("..tostring(OID)..") Area Owner]",true) -- using DoForSessionGameObject, because Area is not known if local player is not in the same session
    ...
    local function OnIslandSettledSessionEnter(PID)
      ...
      if ts.Participants.GetGetCurrentParticipantID() == PID then -- just to make sure its not 4 times for the same local player executed if there are 4 humans (for coop it does not matter because the trigger is only executed once per coop-team, so its fine)
        local SessionGuid = session.getSessionGUID()
          ...
          -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." Update Areas Cache",ModID)
          
          g_AnnoTools.start_thread("OnIslandSettledSessionEnter",ModID,function()
            ...
            -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." before Kontor_OIDs_before",ModID)
            ...
            -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." after Kontor_OIDs_before",ModID)
          -- loop through Areas of current session and save owner+Kontor OID to cache
            g_ObjectFinderSerp.AreasCurrentSessionLooper(function(SessionGuid,ParticipantID,AreaID,SessionID,IslandID,AreaIndex,AreaOwner,Kontor_OID,GUID)
              ...
              g_AnnoTools.modlog(SessionGuid..","..AreaID..","..Kontor_OID,ModID)
            end)
            -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." after AreasCurrentSessionLooper",ModID)
            ...
            if Kontor_OIDs_before~=nil and not g_LuaTools.tables_equal(Kontor_OIDs_before,g_ObjectFinderSerp.ObjectFinderCacheSerp.Kontor_OIDs[curSessionID]) then
                -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." before t_SyncCacheToEveryone",ModID)
              g_ObjectFinderSerp.t_SyncCacheToEveryone()
            end
            
            -- g_AnnoTools.modlog("OnIslandSettledSessionEnter "..tostring(SessionGuid).." Updated Areas Cache Done",ModID)
    ...
    local function OnSessionLoaded(PID)
      if ts.Participants.GetGetCurrentParticipantID() == PID then -- just to make sure its not 4 times for the same local player executed if there are 4 humans (for coop it does not matter because the trigger is only executed once per coop-team, so its fine)
        g_AnnoTools.modlog("OnSessionLoaded Update SessionParticipants Cache",ModID)
        g_AnnoTools.start_thread("OnSessionLoaded",ModID,function()
          g_AnnoTools.waitForTimeDelta(2000) -- small delay to give the game time to finish everything, like spawning SessionParticipants
          -- wait for everything to initial finish
          while g_ObjectFinderSerp==nil or g_SaveLuaStuff_Serp==nil or g_PeersInfo_Serp==nil or g_PeersInfo_Serp.CoopFinished~=true or g_CoopCountRes==nil or g_CoopCountRes.Finished~=true do
            coroutine.yield()
          end
    ...
    local function RegisterEvents() -- not called anymore, since event. does crash too often. using triggers again
      ...
      if event.OnSessionLoaded[ModID] == nil then -- only add it once
        g_AnnoTools.modlog("Register OnSessionLoaded",ModID)
        ...
        event.OnSessionLoaded[ModID] = function(SessionContext)
          system.start(function()
            ...
            if status==false then -- error
              ...
              g_AnnoTools.modlog("ERROR OnSessionLoaded: Function OnSessionLoaded had an error: "..tostring(err),ModID)
            end
          end,ModID.." OnSessionLoaded")
        end
      end
      if event.OnSessionEnter[ModID] == nil then -- only add it once
        g_AnnoTools.modlog("Register OnSessionEnter",ModID)
        was_already_registered = false
        event.OnSessionEnter[ModID] = function(SessionContext)
          system.start(function()
            ...
            if status==false then -- error
              ...
              g_AnnoTools.modlog("ERROR OnSessionEnter: Function OnIslandSettledSessionEnter had an error: "..tostring(err),ModID)
            end
    ...
    g_AnnoTools.start_thread("t_LoadCache",ModID,g_ObjectFinderSerp.t_LoadCache) -- this already updates on game start the Cache SessionParticipants and also LoadedSessions, while OnSessionEnter updates Kontor_OIDs
