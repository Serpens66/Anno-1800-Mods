...
if g_LuaScriptBlockers[ModID]==nil then
    ...
    local function Sync_ActivePeersInfo(Other_ActiveCoopPeers,Other_InactiveCoopPeers)
      ...
      if g_MP_PopUps_Serp.CountInfosReceived == g_CoopCountRes.TotalCount then -- received every info from all peers (everyone completed PopUps)
    ...
    local function OnAllPopUpsAnswered()
        if ts.GameSetup.GetIsMultiPlayerGame() then
          ...
          if g_CoopCountRes~=nil then
            ...
            if g_ObjectFinderSerp~=nil then  -- t_ExecuteFnWithArgsForPeers need to use "Everyone" here, because the receivers might not know yet which PeerInt they are (if not finished PopUps yet).
              g_ObjectFinderSerp.t_ExecuteFnWithArgsForPeers("g_MP_PopUps_Serp.Sync_ActivePeersInfo",nil,false,"Everyone",g_PeersInfo_Serp.ActiveCoopPeers,g_PeersInfo_Serp.InactiveCoopPeers)
            end
          end
        else
    ...
    local function t_Start(everactive_coops)
      if ts.GameSetup.GetIsMultiPlayerGame() then
        if g_LuaTools.table_len(everactive_coops[g_PeersInfo_Serp.PID])>1 then -- if we have at least 2 players in this coop team
