...
print("annotools.lua registered")
local tools_ModID = "shared_LuaTools_Serp"


-- usage:

-- if g_AnnoTools==nil then
  -- console.startScript("data/scripts_serp/annotools.lua")
-- end
  ...
  return system.start(function()
    local status,err = pcall(fn,table.unpack(args))
      if status==false then
        g_AnnoTools.modlog("ERROR in thread '"..tostring(threadname).."': "..tostring(err),ModID)
        error(err)
      end
...
local Fn_Blocker = {}
-- fn_name is a string, eg. "g_AnnoTools.modlog"
...
local function CallGlobalFnBlocked(fn_name,blocknameadd,blocktime,...)
  ...
  g_AnnoTools.start_thread(blockername,tools_ModID,function()
    g_AnnoTools.waitForTimeDelta(blocktime) -- unblock it again, so it can be executed the next time we load a game
    Fn_Blocker[blockername] = nil
  end)
  fn = g_LuaTools.myeval(fn_name)
  return fn(...)
end
  ...
  for _,name in ipairs(threadstostop) do
    if system.internal.coroutines[name]~=nil then
      if coroutine.status(system.internal.coroutines[name])=="suspended" then -- seems to be ok to still close them, but just in case log it
        ...
        g_AnnoTools.modlog("stopthreads, stopping thread although its still busy: "..tostring(name),tools_ModID)
      end
      ...
      -- if coroutine.status(system.internal.coroutines[name])=="suspended" then
        -- system.internal.coroutines[name] = nil
      -- else
        -- print("stopthreads, not stopping thread because its still busy: "..tostring(name),tools_ModID)
        -- g_AnnoTools.modlog("stopthreads, not stopping thread because its still busy: "..tostring(name),tools_ModID)
      -- end
    end
  end
...
local function ToTextembed(path)
  local path_parts = g_LuaTools.mysplit(path,".")
  local path_textemb = ""
...
local function OIDtableToOID(OIDtable)
  ...
  if type(OIDtable)=="table" then 
    ObjectID = g_LuaTools.to_bint(OIDtable["ObjectID"])
    EditorFlag = g_LuaTools.to_bint(OIDtable["EditorFlag"] or 0)
    EditorChunkID = g_LuaTools.to_bint(OIDtable["EditorChunkID"] or 0)
    if type(OIDtable["AreaID"])=="table" then
      AreaID = g_LuaTools.to_bint(AreatableToAreaID(OIDtable["AreaID"]))
    elseif type(OIDtable["AreaID"])=="number" then
      AreaID = g_LuaTools.to_bint(OIDtable["AreaID"])
    end
  end
...
-- local AreaID = g_LuaTools.to_bint(9730)
-- local ObjectID = g_LuaTools.to_bint(4467)
-- local EditorFlag = g_LuaTools.to_bint(1)
-- local EditorChunkID = g_LuaTools.to_bint(0)
-- local solution = g_LuaTools.to_bint("9223413826886570355")
-- for i=1,100 do
...
-- FnViaTextEmbed is used to execute your custom lua code via ThirdPartyButton on CharacterNotification (which can only execute textembeds, not lua code)
  ...
  local function FnViaTextEmbed(PID)
    if PID == ts.Participants.GetGetCurrentParticipantID() then -- only the correct Participant 
      ...
      if TaskString~=oldname then -- only the correct Peer (who hit the button)
        ...
        local tasksplit = g_LuaTools.mysplit(TaskString,"_")
        local funcname = table.remove(tasksplit,1)
        local func = g_LuaTools.myeval(funcname)
        for i=1,#tasksplit do
          local value = tasksplit[i]
          tasksplit[i] = g_LuaTools.myeval(tasksplit[i])
        end
        local success, err = pcall(func,table.unpack(tasksplit))
        if success==false then
          g_AnnoTools.modlog("ERROR FnViaTextEmbed while trying to execute TaskString: "..tostring(TaskString).." , error: "..tostring(err),ModID)
        end
        return success, err
      end
    end
  ...
  local function GetFertilitiesOrLodesFromArea_CurrentSession(AreaID,FertilitiesOrLodesString)
    local count = g_AnnoTools.DoForSessionGameObject("[Area AreaFromID("..tostring(AreaID)..") "..tostring(FertilitiesOrLodesString).." Count]",true)
    local results = {}
    if count~=nil and count~="nil" then
      ...
      if count~=nil and count>0 then
        local GUID
        for i=0,count-1 do
          GUID = g_AnnoTools.DoForSessionGameObject("[Area AreaFromID("..tostring(AreaID)..") "..tostring(FertilitiesOrLodesString).." At("..tostring(i)..") Guid]",true)
  ...
  local function GetVectorGuidsFromSessionObject(ts_embed_string)
    ...
    local count = g_AnnoTools.DoForSessionGameObject(ts_embed_string,true)
    local results = {}
    if count~=nil and count~="nil" then
      count = tonumber(count)
      if count~=nil and count>0 then
        ...
        for i=0,count-1 do
          ...
          GUID = g_AnnoTools.DoForSessionGameObject(ts_embed_string_guid,true)
          GUID = tonumber(GUID)
          if GUID~=nil and GUID~=0 then
            ts_embed_string_value = string.gsub(ts_embed_string, "Count", "At("..tostring(i)..") Value")
            Value = g_AnnoTools.DoForSessionGameObject(ts_embed_string_value,true)
  ...
  -- RefOid = 0 for whole menus , for ships/buildings: g_AnnoTools.get_OID(session.getSelectedFactory()) 
  local function GetCoopPeersAtMarker(UIState,RefOid)
    local count = g_AnnoTools.DoForSessionGameObject("[Online GetCoopPeersAtMarker("..tostring(UIState)..","..tostring(RefOid)..") Count]",true)
    local peerints = {}
    if count~=nil and count~="nil" then
      count = tonumber(count)
      if count~=nil and count>0 then
        local peerint
        for i=0,count-1 do
          peerint = g_AnnoTools.DoForSessionGameObject("[Online GetCoopPeersAtMarker("..tostring(UIState)..","..tostring(RefOid)..") At("..tostring(i)..")]",true)
  ...
  local function GetGameObjectPath(OID,path)
    ...
    if OID~=nil and OID~=0 then
      if type(OID)=="number" then -- GetGameObject can not handle other than number
        ...
        local path_textemb = g_AnnoTools.ToTextembed(path)
        g_AnnoTools.modlog("[MetaObjects SessionGameObject("..tostring(OID)..")"..path_textemb.."]",tools_ModID)
        ret = g_AnnoTools.DoForSessionGameObject("[MetaObjects SessionGameObject("..tostring(OID)..")"..path_textemb.."]",true)
        local status,err = pcall(g_LuaTools.myeval,ret) -- to make "1" 1 or "true" true
  ...
  -- for propid,propname in pairs(g_AnnoTools.PropertiesStringToID) do
    -- local success = g_AnnoTools.HasProperty(userdata,propid)
    -- if success then
  ...
  local function HasWalking(OID,userdata)
    local hasproperty = g_AnnoTools.HasProperty(userdata,"Walking",OID)
    if hasproperty then
      return true
    elseif hasproperty==nil then -- then try the less secure way via GetGameObject, which also works if object is in another session
      return g_AnnoTools.GetGameObjectPath(OID,"Walking.BaseSpeedWithUpgrades") ~= 0
    end
  end
  local function HasCommandQueue(OID,userdata)
    local hasproperty = g_AnnoTools.HasProperty(userdata,"CommandQueue",OID)
    if hasproperty then
      return true
    elseif hasproperty==nil then -- then try the less secure way via GetGameObject, which also works if object is in another session
      return g_AnnoTools.GetGameObjectPath(OID,"CommandQueue.UI_IsNonMoving") or g_AnnoTools.GetGameObjectPath(OID,"CommandQueue.UI_IsMoving") -- scheint immer eins von beiden wahr zu sein, auch zb auf patroullie/traderoute
    end
  end
  local function HasAttacker(OID,userdata)
    local hasproperty = g_AnnoTools.HasProperty(userdata,"Attacker",OID)
    if hasproperty then
      return true
    elseif hasproperty==nil then -- then try the less secure way via GetGameObject, which also works if object is in another session
      return g_AnnoTools.GetGameObjectPath(OID,"Attacker.DPS") ~= 0
    end
  end
  local function HasAttackable(OID,userdata) -- we can not check if and by what it can be attacked though
    local hasproperty = g_AnnoTools.HasProperty(userdata,"Attackable",OID)
    if hasproperty then
      return true
    elseif hasproperty==nil then -- then try the less secure way via GetGameObject, which also works if object is in another session
      return g_AnnoTools.GetGameObjectPath(OID,"Attackable.MaxHitPoints") ~= 0
    end
  end
  local function HasBombarder(OID,userdata) -- most likely also means it is an airship
    local hasproperty = g_AnnoTools.HasProperty(userdata,"Bombarder",OID)
    if hasproperty then
      return true
    elseif hasproperty==nil then -- then try the less secure way via GetGameObject, which also works if object is in another session
      return g_AnnoTools.GetGameObjectPath(OID,"Bombarder.ShaftCount") ~= 0
    end
  end
  local function NeedsBuildPermit(OID,GUID)
    GUID = GUID or OID~=nil and g_AnnoTools.GetGameObjectPath(OID,"GUID")
  ...
  local function AffectedByStatusEffect(OID,StatusEffectGUID) -- eg StatusEffect dealt by projectiles. this way you can easily filter for objects hit with your custom projectile
    return g_AnnoTools.GetGameObjectPath(OID,"Attackable.GetIsPartOfActiveStatusEffectChain("..tostring(StatusEffectGUID)..")") -- unfortunately does not work with buffs provided in a different way
  end
...
if g_LuaTools==nil then -- also start this, so its enough to call one of it on your script
