...
if event.OnLeaveUIState[ModID] == nil then -- only add it once (per anno game start)
  ...
  ts.GameClock.SetSetGameSpeed = function(new) -- no need to do it for IncreaseGameSpeed/DecreaseGameSpeed because they dont work well anyways (once slowest is reached, you can not increase anymore, so no modder should use them anyways)
    -- g_LTL_Serp.modlog("SetSetGameSpeed called with: "..tostring(new),ModID)
  ...
  g_LTL_Serp.start_thread("LoadingScreenLeft Execute_SavegameLoadedEvent",ModID,t_OnGameLoaded)
