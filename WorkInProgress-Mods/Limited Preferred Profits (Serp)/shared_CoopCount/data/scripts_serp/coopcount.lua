...
-- same script is used within LuaMediumTools mod, beside that this one was made independend of mediumtools
...
if g_LuaScriptBlockers[ModID]==nil then
    ...
    -- the _nodouble_ within the threadname makes sure that it does nothing if a thread named like this is currently still running
    local function MakeNewCount()
      g_LTL_Serp.modlog("MakeNewCount",ModID)
      g_LTL_Serp.start_thread("MakeNewCount_nodouble_",ModID,t_DoMakeNewCount)
    end
    ...
    g_LTL_Serp.start_thread("First_MakeNewCount",ModID,function()
