...
local BadNewsArticles = {1500002502,1500002503,1500002504,1500002505,1500002506,1500002507,1500002508,1500002509}
...
if g_LuaScriptBlockers[ModID]==nil then
    ...
    local function BuyShareForDoublePrice(PID)
      if PID == ts.Participants.GetGetCurrentParticipantID() then
        local continue = g_LTM_Serp==nil and true or g_LTM_Serp.ContinueCoopCalled()
        if continue then -- either its the first peer or we dont know
    ...
    -- will be executed multiple times in coop I guess.. but its ok, bad newspapers are not that terrible, since you can use Propaganda
    local function AddBadNews(PID)
      if PID == ts.Participants.GetGetCurrentParticipantID() then
        local continue = g_LTM_Serp==nil and true or g_LTM_Serp.ContinueCoopCalled()
        if continue then -- either its the first peer or we dont know
          local Arcticle = g_LTL_Serp.random_choice(BadNewsArticles)
          if Arcticle~=nil then
            ts.Newspaper.AddNewsArticle(Arcticle)
          end
        end
      end
    end
