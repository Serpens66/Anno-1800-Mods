...
-- if g_saveloaded_events_serp~=nil and g_saveloaded_events_serp[ModID]==nil then
...
-- My Execute_SavegameLoadedEvent function calls all event-functions added by other mods, 5 seconds after a savegame was loaded. 
-- (It may be the very same game though, eg. go to main menu and back into game).
-- But it is for sure better to execute your possibly expensive code with help of this event, instead on every single SessionEnter, if it is enough to call it once per savegame load
...
-- and when using the event directly, one needs to define a global variable/global function for each mod using this, because there is no other way to make sure your function is not added multiple times.
--   while when using g_saveloaded_events_serp this is not needed, because I work with ModID as key to check if it was added (while event. can only check value, which is a different object each time for local functions/variables)
-- advantage of using event. would be of course to being able to call local functions/variables (although I wonder why this is possible)
